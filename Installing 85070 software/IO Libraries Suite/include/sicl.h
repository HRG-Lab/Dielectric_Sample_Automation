...
/* Copyright 1992-2016 Keysight Technologies Inc.  All Rights Reserved. */
/*
 * Keysight Standard Instrument Control Library
 *
 * This file conforms to the Keysight SICL Spec as specified in I_SICL_REVISION.
 *
 ...
 */
#ifndef _I_INT64_UINT64_DEFINED
#if defined(_WIN64) || defined(WIN64) || ((defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)) && !defined(_NI_mswin16_))
...
#define _I_INT64_UINT64_DEFINED
#if defined(_WIN64)
#pragma message ("Note: This is 64-bit SICL build")
#define _SICL_WIN64
typedef void *             IHandle;
typedef unsigned   __int64 IBusAddress;
typedef unsigned   __int64 IBusSize;
typedef unsigned   __int64 IPageStart;
typedef unsigned   __int64 IPageCount;
#else
/* This is a 32-bit OS, not a 64-bit OS */
#pragma message ("Note: This is 32-bit SICL build")
#define _SICL_WIN32
...
#else
#if defined(_SICL_WIN64)
#define SICLCALLBACK __fastcall
#define SICLAPI __fastcall
#define SICLAPIV __fastcall
#else
#if defined(_MS_DOS_WIN)
 ...
 * Support levels:
 */
#define I_SICL_REVISION 48  /* Keysight SICL Revision 4.8 */
...
#define I_INTF_HISLIP  15
#define I_INTF_PXI     16
...
#define I_TRIG_TTL8        0x00000010L
#define I_TRIG_TTL9        0x00000020L
#define I_TRIG_TTL10       0x00000040L
#define I_TRIG_TTL11       0x00000080L
 ...
 */
#if defined(_MS_DOS_WIN) || defined(_SICL_WIN32) || defined(_SICL_LYNX_LE) || defined (_SICL_LINUX_LE) || defined(_SICL_WIN64)
   /* Little Endian */
#define I_ORDER_LE
#undef  I_ORDER_BE
#endif
...
#define I_SERIAL_FLOW_DTR_DSR4    4
#define I_SERIAL_FLOW_RTS_CTS_XON 5
#define I_SERIAL_FLOW_DTR_DSR_XON 6
...
typedef enum 
{
	I_MAP_PXI_BAR0_MEM = 0x0,	// Value matches the value the kernel driver uses for BAR0. 
	I_MAP_PXI_BAR1_MEM,			// Value matches the value the kernel driver uses for BAR1. 
	I_MAP_PXI_BAR2_MEM,
	I_MAP_PXI_BAR3_MEM,
	I_MAP_PXI_BAR4_MEM,
	I_MAP_PXI_BAR5_MEM,
...
/* These correspond to the VISA address types */
#define I_PXI_ADDR_NONE (0)
#define I_PXI_ADDR_MEM  (1)
#define I_PXI_ADDR_IO   (2)
#define I_PXI_ADDR_CFG  (3)
...
#define I_ERR_LINEINUSE      36
#define I_ERR_TRIGNMAPPED    37
#define I_ERR_INVOFFSET      38
#define I_ERR_INVSIZE        39
#define I_ERR_NENABLED       40
#define I_ERR_QUEUE_OVERFLOW 41
#define I_ERR_PARAM_STATE    42
#if !defined(STD_SICL)
...
#define I_LUINFO_FLAG_MULTIINST 0x00000004
...
/* PXI Definitions (obsolete - use pxiinfo2) */
typedef struct {
   int            structSize;    /* size of pxiinfo in bytes */
   unsigned short manufId;
   unsigned short modelCode;
   char           manufName[256];
   char           modelName[256];
   unsigned short intfNumber;      /* 0 - 255 */
   unsigned short busNumber;       /* 0 - 255 */
   unsigned short devNumber;       /* 0 - 31 */
   unsigned short funcNumber;      /* 0 - 7 */
   char           slotPath[256];
   short          slotLbusLeft;    /* 0 - 32767, UNKNOWN(-1) */
   short          slotLbusRight;   /* 0 - 32767, UNKNOWN(-1) */
   short          trigBus;         /* 0 - 32767, UNKNOWN(-1) */
   short          starTrigBus;     /* 0 - 32767, UNKNOWN(-1) */
   short          starTrigLine;    /* 0 - 32767, UNKNOWN(-1) */
   unsigned short memTypeBar[6];   /* NONE(0), MEMORY(1), IO(2) */
   IBusAddress    memBaseBar[6];
   IBusSize       memSizeBar[6];
   IBusAddress    memBaseCfg;
   IBusSize       memSizeCfg;
   short          chassisNumber;   /* 0 - 255, UNKNOWN(-1) */
   short          isPxiExpress;    /* TRUE(1), FALSE(0) */
   short          slotLwidth;      /* 1,4,8 */
   short          maxLwidth;       /* 1,4,8 */
   short          actualLwidth;    /* 1,4,8 */
   short          dStarBus;        /* 0 - 32767, UNKNOWN(-1) */
   short          dStarSet;        /* 0 - 23767, UNKNOWN(-1) */
   short          slotNumber;      /* 0 - ?????, UNKNOWN(-1) */
   short          isSimulated;     /* TRUE(1), FALSE(0) */
   short          isAxie;          /* TRUE(1), FALSE(0) */
   short          isKeysightDev;   /* TRUE(1), FALSE(0) */
   short          isManualConfig;  /* TRUE(1), FALSE(0) */
   short          is64BitBar;      /* TRUE(1), FALSE(0) */
   short          driverInstalled; /* TRUE(1), FALSE(0) */
   short          verified;        /* TRUE(1), FALSE(0) */
   short          visible;         /* TRUE(1), FALSE(0) */
   short          reserved3h;
   int            reserved4;
} pxiinfo_old;

/* PXI Definitions with 64-bit BAR values*/
typedef struct {
   int            structSize;      /* size of pxiinfo2 in bytes */
   short          isSimulated;     /* TRUE(1), FALSE(0) */
   short          isKeysightDev;   /* TRUE(1), FALSE(0) */
   short          driverInstalled; /* TRUE(1), FALSE(0) */
   short          verified;        /* TRUE(1), FALSE(0) */
   ...
   short          slotNumber;      /* 0 - ?????, UNKNOWN(-1) */
   unsigned short memTypeBar[6];   /* NONE(0), MEMORY(1), IO(2) */
   I_UInt64       memBaseBar[6];
   I_UInt64       memSizeBar[6];
   I_UInt64       memBaseCfg;
   I_UInt64       memSizeCfg;
   short          slotLbusLeft;    /* 0 - 32767, UNKNOWN(-1) */
   short          slotLbusRight;   /* 0 - 32767, UNKNOWN(-1) */
...
#define SICL_UNKNOWN -1
#define SICL_INVALID_INDEX_VALUE 0xffffffff
...
#if defined(__STDC__) || defined(__cplusplus) || defined(_MS_DOS_WIN) || defined (_SICL_WIN32) || defined(_SICL_WIN64)
...
#if defined(_MS_DOS_WIN) || defined(_SICL_WIN32) || defined(_SICL_WIN64)
...
/* extended map/peek/poke/move routines */
   IHandle SICLAPI imapx(INST id, int mapspace, IPageStart pagestart, IPageCount pagecnt);
   int SICLAPI iunmapx(INST id, IHandle handle, int mapspace, IPageStart pagestart, IPageCount pagecnt);
   int SICLAPI iderefptr(INST id, IHandle handle, unsigned char *value);
   int SICLAPI ipeekx8( INST id, IHandle handle, IBusAddress offset, unsigned char  *value);
   int SICLAPI ipeekx16(INST id, IHandle handle, IBusAddress offset, unsigned short *value);
   int SICLAPI ipeekx32(INST id, IHandle handle, IBusAddress offset, unsigned long  *value);
   int SICLAPI ipokex8( INST id, IHandle handle, IBusAddress offset, unsigned char  value);
   int SICLAPI ipokex16(INST id, IHandle handle, IBusAddress offset, unsigned short value);
   int SICLAPI ipokex32(INST id, IHandle handle, IBusAddress offset, unsigned long  value);
#if defined (_SICL_VXI_D64)
   int SICLAPI ipeekx64(INST id, IHandle handle, IBusAddress offset, unsigned __int64 *value);
   int SICLAPI ipokex64(INST id, IHandle handle, IBusAddress offset, unsigned __int64 value);
#endif
   int SICLAPI iblockmovex(INST id,
                      IHandle       src_handle,
                      IBusAddress   src_offset,
                      int           src_width,
                      int           src_increment,
                      IHandle       dest_handle,
                      IBusAddress   dest_offset,
                      int           dest_width,
                      int           dest_increment,
                      IBusSize	    cnt,
                      int           swap
...
/* Map routines */
char _far * SICLAPI imap(INST id,int mapspace,IPageStart pagestart,IPageCount pagecnt,char _far *suggested);
int SICLAPI iunmap(INST id,char _far *addr,int mapspace,IPageStart pagestart,IPageCount pagecnt);
...
#if !defined(STD_SICL)
   int SICLAPI itrace(unsigned long level); /* Turn on tracing */
...
#else /* __STDC__ || __cplusplus || _MS_DOS_WIN || _SICL_WIN32 ||_SICL_WIN64 */
...
#endif /* __STDC__ || __cplusplus || _MS_DOS_WIN || _SICL_WIN32 || _SICL_WIN64 */
